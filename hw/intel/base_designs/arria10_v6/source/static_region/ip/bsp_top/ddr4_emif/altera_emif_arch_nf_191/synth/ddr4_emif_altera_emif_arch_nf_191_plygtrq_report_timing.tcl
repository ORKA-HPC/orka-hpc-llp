...
if { ! [timing_netlist_exist] } {
   ...
   source "$script_dir/ddr4_emif_altera_emif_arch_nf_191_plygtrq_report_timing_core.tcl"
   if { ! [timing_netlist_exist] } {
...
ddr4_emif_altera_emif_arch_nf_191_plygtrq_initialize_ddr_db ddr_db var
...
foreach inst $instances {
   ...
   if {[llength $instances] <= 1} {
      set fbasename "${::GLOBAL_ddr4_emif_altera_emif_arch_nf_191_plygtrq_corename}"
   } else {
      set fbasename "${::GLOBAL_ddr4_emif_altera_emif_arch_nf_191_plygtrq_corename}_${inst_id}"
   }
   ...
   ddr4_emif_altera_emif_arch_nf_191_plygtrq_perform_read_capture_analysis $opcname $inst pins var summary

   #######################################
   # Write Analysis

   ddr4_emif_altera_emif_arch_nf_191_plygtrq_perform_write_launch_analysis $opcname $inst pins var summary

   #######################################
   # Address/command Analyses

   ddr4_emif_altera_emif_arch_nf_191_plygtrq_perform_ac_analysis  $opcname $inst pins var summary

   #######################################
   # DQS Gating Analysis

   ddr4_emif_altera_emif_arch_nf_191_plygtrq_perform_dqs_gating_analysis   $opcname $inst pins var summary

   #######################################
   # Write Levelling Analysis

   ddr4_emif_altera_emif_arch_nf_191_plygtrq_perform_write_levelling_analysis $opcname $inst pins var summary

   #######################################
   # Analyses related to FPGA core

   ddr4_emif_altera_emif_arch_nf_191_plygtrq_perform_core_analysis $opcname $inst pins var summary

   #######################################
   # Print out the Summary Panel for this instance

   set summary [lsort -command ddr4_emif_altera_emif_arch_nf_191_plygtrq_sort_proc $summary]

   set f -1
   set fname "${fbasename}_summary.csv"

   if { [ddr4_emif_altera_emif_arch_nf_191_plygtrq_get_operating_conditions_number] == 0 } {
      set f [open $fname w]

      puts $f "Core: ${::GLOBAL_ddr4_emif_altera_emif_arch_nf_191_plygtrq_corename} - Instance: $inst"
   ...
   post_message -type info "Core: ${::GLOBAL_ddr4_emif_altera_emif_arch_nf_191_plygtrq_corename} - Instance: $inst"
   ...
   foreach summary_line $summary {
      ...
      if {$su != "--"} {
         set su [ ddr4_emif_altera_emif_arch_nf_191_plygtrq_round_3dp $su]
      }
      if {$hold != "--"} {
         set hold [ ddr4_emif_altera_emif_arch_nf_191_plygtrq_round_3dp $hold]
      }
